import requests
from helpers.constants import EAMID, TENANT, USER_FUNCTION_NAME, SYSTEM_FUNCTION_NAME, WINDOW_, USERID, PASSWORD, EWSLanguage, ISNAMEDUSER, HXGN_BASE_URL, USERGROUP, CURRENT_DIR
...
def getSessionCookies():
    ...
    global EAMID; TENANT; USER_FUNCTION_NAME; SYSTEM_FUNCTION_NAME; WINDOW_; USERID; PASSWORD;EWSLanguage;ISNAMEDUSER;HXGN_BASE_URL; CURRENT_DIR
    ...
    "referrer": "https://eam-u2alnxapp.metrolinxuat.com/web/base/logindisp"
  }
    payload = "USER_FUNCTION_NAME=LOGIN&SYSTEM_FUNCTION_NAME=LOGIN&window=main_eam&userid=MELISSALU&password=!Metr0_linx%23&tenant=DS_MP_1"

    with requests.Session() as session:
        # post = session.post(URL, verify=False)
        # post = session.post(f"{HXGN_BASE_URL}/web/base/logindisp", json=payload, verify=False)
        post = session.post(f"https://eam-u2alnxapp.metrolinxuat.com/web/base/login?",headers=headers, data=payload, verify=False)
        # print(post.text)
        # post = session.post(f"{HXGN_BASE_URL}/web/base/login?eamid=bd4761d2-fe6c-4530-89e3-891c0e8e4061", json=payload, verify=False)
        get = session.get('https://eam-u2alnxapp.metrolinxuat.com/web/base/COMMON',verify=False)
        print(get.content)
        # print(post.content)
...
def getRequest(URL_list, referer_=None):
    """
    Return GET response as JSON
    """
    global cookies, referer
...
def postRequest(URL, payload,referer_=None):
    """
    Return POST response as JSON
    """

    global cookies, USERID, referer
    if referer == None:
        referer_= referer

    # "Cookie": "BIGipServerEAM-U2ALNXAPP-POOL_443=996915210.47873.0000;JSESSIONID=MWTYb8jYX7RIgLTrsYpqEXYlArpQnKomaSoPUSGD.tomcat01",
    # "Referer":referer_,
    data_header = {
            "Cookie": cookies,
            "Content-Type": "application/x-www-form-urlencoded; charset=UTF-8",
...
def postStandardWO():
    ...
    payload_list = [
                ...
                f"&LOV_ALIAS_VALUE_12=ANALYST-FM",
                f"&LOV_ALIAS_TYPE_12=text",
                f"&eamid=2028a9eb-8595-4edb-b006-7edafcb7781e",
                f"&tenant=DS_MP_1"
    ]
    payload = "".join(payload_list)
...
def postPriority():
    ...
    payload_list = [
        ...
        f"&GRIDNAME=WOBFMM",
        f"&TAGNAME=priority",
        f"&eamid=e3ffa766-9efd-4839-91b0-43a3b7c1e958",
        f"&tenant=DS_MP_1"
    ]
    payload = "".join(payload_list)
...
def postParentWO():
    ...
    payload_list = [
            ...
            f"&LOV_ALIAS_TYPE_3=text",
            f"&eamid=f2d81eb0-9465-47ac-a971-902672f52591",
            f"&tenant=DS_MP_1"
    ]
...
def postNewWORecord():
    ...
    payload_list = [
                        ...
                        f"&actudfnum03=",
                        f"&actudfnum04=",
                        f"&currentdate=06%2F30%2F2023%2000%3A00",
                        f"&actudfnum05=",
                        f"&tempstandardwo=",
                        f"&tempcasetype=",
                        f"&actudfnum01=",
                        ...
                        f"&workordernum=%3CAuto-Generated%3E",
                        f"&dependant=-1",
                        ...
                        f"&multiequip=0",
                        f"&datereported=06%2F30%2F2023%2014%3A10",
                        f"&esignaturedate=",
                        f"&schedgroup=",
                        f"&smda=0",
                        f"&projbud=",
                        ...
                        f"&equipmentdesc=SuperLo%20Enviro%20500%3B%20DDS-Electric%3B%204000",
                        f"&safetyreviewedby=",
                        f"&fromxcoordinate=",
                        f"&campaign=",
                        f"&actenddate=",
                        ...
                        f"&udfchar22=",
                        f"&udfchar20=",
                        f"&schedstartdate=06%2F30%2F2023",
                        f"&problemcode=",
                        f"&phonenumber=",
                        f"&locationdesc=Bus%204000%3B%20DDS-Electric",
                        f"&actioncode=",
                        f"&supplierorg=",
                        f"&reportedby=MELISSALU",
                        f"&customercontract=",
                        f"&originatingjob=",
                        f"&manufacturer=",
                        ...
                        f"&udfchar04=",
                        f"&longitude=",
                        f"&udfchar12=BFM-BAE-STEEPROCK",
                        f"&udfchar13=",
                        f"&udfchar10=MX",
                        f"&package=",
                        f"&udfchar11=",
                        f"&serialnumber=SFES11119LGN60001",
                        f"&workordertype=COR",
                        f"&workordertype_display=Corrective%20Maintenance",
                        f"&equipment=BUS-4000",
                        ...
                        f"&temptopoint=",
                        f"&tempfrompoint=",
                        f"&PKID=%3CAuto-Generated%3E%23BRT%23",
                        f"&eamid=f2d81eb0-9465-47ac-a971-902672f52591",
                        f"&tenant=DS_MP_1"
    ]
    payload = "".join(payload_list)
